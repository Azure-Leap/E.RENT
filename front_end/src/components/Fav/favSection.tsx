...
const ProductItem = ({ item, product }: any) => {
  ...
  const { deleteCard }: any = useContext(CartContext);
    ...
    <div className="container mx-auto">
      <div className="p-2">
        <div className="grid grid-cols-3 gap-5 p-1">
          <div>
            <img src={item.product.imgUrl} alt="picture" className="object-cover border border-black w-full h-64 p-3" />
          </div>
          <div className="text-gray-700">
            <Link className="lg:font-semibold text-2xl sm:text-md font-semibold max-sm:text-xs" href={`Products/${item?.product._id}`}>
              {item?.product?.title}
            </Link>

            <p className="font-extralight text-md max-sm:text-xs ">{item?.product?.description}</p>
            <p className="font-extralight text-xs text-gray-500">{item?.product?.rent_start_day}</p>
          </div>

          <div className="flex justify-end sm:text-sm md:text-md sm:flex-row">
            <div className="font-normal text-2xl text-gray-700">{item.totalPrice}₮</div>
          </div>
        </div>
...
const FavSection = () => {
  const { favItems, addItemToCart }: any = useContext(CartContext);
    ...
    <div className="h-screen bg-white">
      ...
      {favItems && (
        <>
          <div className="flex justify-center p-5 sm:flex-row">
            <ol className="flex items-center justify-center space-x-2  font-medium text-center text-xl ">
              <li className="flex items-center text-cyan-600 ">
                <span className="hidden sm:inline-flex sm:ml-2">Таалагдсан түрээс</span>
          ...
          <div className="px-44">
            <div className="md:col-span-3 border-2 shadow-md p-2">{favItems?.length && favItems.map((el: any, i: any) => <ProductItem key={i} item={el} />)}</div>
          </div>
        </>
      )}
    </div>
